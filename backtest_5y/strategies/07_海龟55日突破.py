...
策略描述：
  当收盘价突破55日新高时买入；当收盘价跌破20日新低或者亏损超过10%时卖出；最长持有120天。
...
PARAMS = {
    ...
    "stop_loss":   0.1,            # 止损，0=关闭
    "max_bars":    120,            # 最长持有交易日，0=不限
...
def prepare(S):
    ...
    for n in (5, 10):
        S["vma%d" % n] = sma(v, n)
    S["rsi6"] = rsi(c, 6)
    S["hh55"] = roll_max(S["high"], 55)
    S["ll20"] = roll_min(S["low"], 20)
...
def buy_signal(S, i):
    ...
    return ((ok(S["hh55"][i - 1]) and S["close"][i] > S["hh55"][i - 1]))
...
def sell_signal(S, i, pos):
    ...
    if ((ok(S["ll20"][i - 1]) and S["close"][i] < S["ll20"][i - 1])):
        return True, "信号"
    return False, ""
