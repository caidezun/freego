...
策略描述：
  当5日均线大于10日均线并且10日均线大于20日均线并且回踩10日均线并且成交量低于5日均量的0.6倍时买入；当收盘价跌破10日均线或者盈利超过15%或者亏损超过5%时卖出；最长持有20天。
...
已识别的规则：
  ...
  卖出（满足任一）：
    - 收盘价 < MA10
  风控：止盈 15.0% · 止损 5.0% · 最长持有 20 个交易日
...
PARAMS = {
    ...
    "slip_permil": 0.5,            # 滑点 ‰
    "take_profit": 0.15,           # 止盈，0=关闭
    "stop_loss":   0.05,           # 止损，0=关闭
    "max_bars":    20,             # 最长持有交易日，0=不限
...
def buy_signal(S, i):
    ...
    return ((ok(S["ma5"][i]) and ok(S["ma10"][i]) and S["ma5"][i] > S["ma10"][i])
            and (ok(S["ma10"][i]) and ok(S["ma20"][i]) and S["ma10"][i] > S["ma20"][i])
            and (ok(S["close"][i]) and ok(S["ma10"][i] * 0.98) and S["close"][i] > S["ma10"][i] * 0.98)
            and (ok(S["close"][i]) and ok(S["ma10"][i] * 1.02) and S["close"][i] < S["ma10"][i] * 1.02)
            and (ok(S["vma5"][i]) and S["volume"][i] < S["vma5"][i] * 0.6))
