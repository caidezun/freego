...
策略描述：
  当收盘价突破布林上轨并且成交量放大1.5倍时买入；当收盘价跌破布林中轨或者亏损超过8%时卖出；最长持有30天。
...
def buy_signal(S, i):
    ...
    return ((ok(S["vma5"][i]) and S["volume"][i] > S["vma5"][i] * 1.5)
            and (ok(S["close"][i]) and ok(S["boll_up"][i]) and S["close"][i] > S["boll_up"][i]))
...
def sell_signal(S, i, pos):
    ...
    if ((ok(S["close"][i]) and ok(S["boll_mid"][i]) and S["close"][i] < S["boll_mid"][i])):
        return True, "信号"
    return False, ""
