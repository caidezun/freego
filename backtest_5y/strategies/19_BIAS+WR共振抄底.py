...
策略描述：
  当BIAS20小于-8并且WR14大于90时买入；当BIAS20大于8或者WR14小于10或者亏损超过8%时卖出；最长持有20天。
...
def buy_signal(S, i):
    ...
    return ((ok(S["bias20"][i]) and S["bias20"][i] < -8)
            and (ok(S["wr14"][i]) and S["wr14"][i] > 90))
...
def sell_signal(S, i, pos):
    ...
    if ((ok(S["bias20"][i]) and S["bias20"][i] > 8)
            or (ok(S["wr14"][i]) and S["wr14"][i] < 10)):
        return True, "信号"
    return False, ""
