...
策略描述：
  当涨幅大于3%并且成交量放大2倍并且收盘价在20日均线之上时买入；当盈利超过10%或者亏损超过6%时卖出；最长持有15天。
...
已识别的规则：
  ...
  卖出（全部满足）：
    - （无信号卖出条件，仅靠风控退出）
  风控：止盈 10.0% · 止损 6.0% · 最长持有 15 个交易日
...
PARAMS = {
    ...
    "slip_permil": 0.5,            # 滑点 ‰
    "take_profit": 0.1,            # 止盈，0=关闭
    "stop_loss":   0.06,           # 止损，0=关闭
    "max_bars":    15,             # 最长持有交易日，0=不限
...
def buy_signal(S, i):
    ...
    return ((ok(S["close"][i]) and ok(S["ma20"][i]) and S["close"][i] > S["ma20"][i])
            and (ok(S["chg"][i]) and S["chg"][i] > 3)
            and (ok(S["vma5"][i]) and S["volume"][i] > S["vma5"][i] * 2))
