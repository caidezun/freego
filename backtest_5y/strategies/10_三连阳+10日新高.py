...
策略描述：
  当连续3天上涨并且收盘价突破10日新高时买入；当盈利超过12%或者亏损超过6%时卖出；最长持有15天。
...
已识别的规则：
  ...
  卖出（全部满足）：
    - （无信号卖出条件，仅靠风控退出）
  风控：止盈 12.0% · 止损 6.0% · 最长持有 15 个交易日
...
def buy_signal(S, i):
    ...
    return ((ok(S["hh10"][i - 1]) and S["close"][i] > S["hh10"][i - 1])
            and (streak(S["close"], i, 3, True)))
