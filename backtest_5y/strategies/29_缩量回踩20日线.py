...
策略描述：
  当5日均线大于20日均线并且回踩20日均线并且成交量低于5日均量的0.7倍时买入；当盈利超过12%或者亏损超过6%时卖出；最长持有20天。
...
已识别的规则：
  ...
  卖出（全部满足）：
    - （无信号卖出条件，仅靠风控退出）
  风控：止盈 12.0% · 止损 6.0% · 最长持有 20 个交易日
...
PARAMS = {
    ...
    "slip_permil": 0.5,            # 滑点 ‰
    "take_profit": 0.12,           # 止盈，0=关闭
    "stop_loss":   0.06,           # 止损，0=关闭
...
def buy_signal(S, i):
    ...
    return ((ok(S["ma5"][i]) and ok(S["ma20"][i]) and S["ma5"][i] > S["ma20"][i])
            and (ok(S["close"][i]) and ok(S["ma20"][i] * 0.98) and S["close"][i] > S["ma20"][i] * 0.98)
            and (ok(S["close"][i]) and ok(S["ma20"][i] * 1.02) and S["close"][i] < S["ma20"][i] * 1.02)
            and (ok(S["vma5"][i]) and S["volume"][i] < S["vma5"][i] * 0.7))
