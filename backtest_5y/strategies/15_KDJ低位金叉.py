...
策略描述：
  当KDJ金叉并且K值小于30时买入；当KDJ死叉或者盈利超过15%或者亏损超过7%时卖出；最长持有30天。
...
已识别的规则：
  ...
  卖出（满足任一）：
    - KDJ 死叉
  风控：止盈 15.0% · 止损 7.0% · 最长持有 30 个交易日
...
def buy_signal(S, i):
    ...
    return ((cross_up(S["kdj_k"], S["kdj_d"], i))
            and (ok(S["kdj_k"][i]) and S["kdj_k"][i] < 30))
