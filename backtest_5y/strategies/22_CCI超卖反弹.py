...
策略描述：
  当CCI14小于-100时买入；当CCI14大于100或者亏损超过7%时卖出；最长持有20天。
...
def buy_signal(S, i):
    ...
    return ((ok(S["cci14"][i]) and S["cci14"][i] < -100))
...
def sell_signal(S, i, pos):
    ...
    if ((ok(S["cci14"][i]) and S["cci14"][i] > 100)):
        return True, "信号"
    return False, ""
