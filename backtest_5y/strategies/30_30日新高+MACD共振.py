...
策略描述：
  当收盘价突破30日新高并且MACD金叉并且成交量放大1.5倍时买入；当MACD死叉或者亏损超过8%时卖出；最长持有45天。
...
PARAMS = {
    ...
    "stop_loss":   0.08,           # 止损，0=关闭
    "max_bars":    45,             # 最长持有交易日，0=不限
...
def buy_signal(S, i):
    ...
    return ((cross_up(S["macd_dif"], S["macd_dea"], i))
            and (ok(S["hh30"][i - 1]) and S["close"][i] > S["hh30"][i - 1])
            and (ok(S["vma5"][i]) and S["volume"][i] > S["vma5"][i] * 1.5))
...
def sell_signal(S, i, pos):
    ...
    if ((cross_dn(S["macd_dif"], S["macd_dea"], i))):
        return True, "信号"
    return False, ""
