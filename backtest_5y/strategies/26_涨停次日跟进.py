...
策略描述：
  当近1个交易日只有1个涨停并且换手率大于5%时买入；当盈利超过8%或者亏损超过5%时卖出；最长持有5天。
...
def prepare(S):
    ...
    S["lu1"] = limit_up_count(S["chg"], h, c, 1, _lp)
...
def buy_signal(S, i):
    ...
    return ((ok(S["lu1"][i]) and S["lu1"][i] == 1)
            and (ok(S["turnover"][i]) and S["turnover"][i] > 5))
