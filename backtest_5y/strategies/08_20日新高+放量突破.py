...
策略描述：
  当收盘价突破20日新高并且成交量放大2倍时买入；当收盘价跌破10日均线或者盈利超过25%或者亏损超过8%时卖出；最长持有30天。
...
已识别的规则：
  ...
  卖出（满足任一）：
    - 收盘价 < MA10
  风控：止盈 25.0% · 止损 8.0% · 最长持有 30 个交易日
...
def buy_signal(S, i):
    ...
    return ((ok(S["hh20"][i - 1]) and S["close"][i] > S["hh20"][i - 1])
