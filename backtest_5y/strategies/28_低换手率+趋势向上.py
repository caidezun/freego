...
策略描述：
  当换手率小于1%并且收盘价在20日均线之上并且5日均线大于20日均线时买入；当收盘价跌破20日均线或者亏损超过7%时卖出；最长持有40天。
...
PARAMS = {
    ...
    "stop_loss":   0.07,           # 止损，0=关闭
    "max_bars":    40,             # 最长持有交易日，0=不限
...
def buy_signal(S, i):
    ...
    return ((ok(S["ma5"][i]) and ok(S["ma20"][i]) and S["ma5"][i] > S["ma20"][i])
            and (ok(S["close"][i]) and ok(S["ma20"][i]) and S["close"][i] > S["ma20"][i])
            and (ok(S["turnover"][i]) and S["turnover"][i] < 1))
...
def sell_signal(S, i, pos):
    ...
    if ((ok(S["close"][i]) and ok(S["ma20"][i]) and S["close"][i] < S["ma20"][i])):
        return True, "信号"
    return False, ""
