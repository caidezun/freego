...
策略描述：
  当近5个交易日只有1个涨停并且BIAS5小于-3并且成交量低于5日均量的0.8倍时买入；当盈利超过10%或者亏损超过6%时卖出；最长持有10天。
...
已识别的规则：
  ...
  卖出（全部满足）：
    - （无信号卖出条件，仅靠风控退出）
  风控：止盈 10.0% · 止损 6.0% · 最长持有 10 个交易日
...
def buy_signal(S, i):
    ...
    return ((ok(S["bias5"][i]) and S["bias5"][i] < -3)
            and (ok(S["lu5"][i]) and S["lu5"][i] == 1)
            and (ok(S["vma5"][i]) and S["volume"][i] < S["vma5"][i] * 0.8))
