...
策略描述：
  当RSI14大于55并且收盘价在20日均线之上时买入；当RSI14小于45或者亏损超过7%时卖出；最长持有30天。
...
def buy_signal(S, i):
    ...
    return ((ok(S["rsi14"][i]) and S["rsi14"][i] > 55)
...
def sell_signal(S, i, pos):
    ...
    if ((ok(S["rsi14"][i]) and S["rsi14"][i] < 45)):
        return True, "信号"
    return False, ""
